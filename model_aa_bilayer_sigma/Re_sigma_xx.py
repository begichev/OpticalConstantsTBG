...
def calc_optical_conductivity(omegas=np.arange(0, 1.5, 0.05), gamma=0.002*t, e_win=0.02*t, save_to='aa_sigma.txt'):
    kstep0 = 0.001
    gamma = 0.02
    t_layers = 0.2
    steps = int(2/3/kstep0)
    energies_load_from='aa_bilayer_energies_'+str(kstep0)+'interlayerhop'+str(t_layers)+'.txt'
    wavefunctions_load_from='aa_bilayer_wavefunctions_'+str(kstep0)+'interlayerhop'+str(t_layers)+'.txt'
    sigma_save_to = 'aa_bilayer_sigma'+str(kstep0)+'_gamma'+str(gamma)+'interlayerhop'+str(t_layers)+'.txt'
